...
useEffect(() => {
  ...
  componentDidMount() {
    fetchAlleles(this.state.locus);
  }

  const fetchAlleles = (locus) => {
    request
      ...
      case "Autosom":
        component = this.state.autosom_locus.map((locus) => (
          <Row type="flex" justify="start" align="top">
            <Radio style={radioStyle} value={locus}>
              {locus}
            </Radio>
          </Row>
        ));
    ...
    return (
      <Radio.Group
        onChange={(e) => {
          ...
          this.fetchAlleles(e.target.value);
        }}
      ...
      <div style={wrapperStyles}>
        <Row>
          <Col span={6} pull={0}>
            <Radio.Group
              value={chromosome}
              onChange={(e) => this.setState({ chromosome: e.target.value })}
            >
              ...
              <ZoomableGroup center={[100, 12.8]}>
                ...
                <Markers>
                  {this.state.alleles.map((allele, i) => (
                    <Marker
                      key={i}
                      marker={{ coordinates: [allele.x, allele.y] }}
                    >
                      <circle
                        cx={0}
                        cy={0}
            ...
            <div style={{ textAlign: "left" }}>
              ...
              {Object.keys(this.state.colorFlag).map((key, index) => (
                <div style={{ textAlign: "left" }}>
                  <Text>
                    &nbsp;
                    <HeartIcon style={{ color: this.state.colorFlag[key] }} />
